...
server <- function(input, output, session) {
  ...
  # Forecasts for horizon defined by the slider
  forecasts <- reactive({
    mdl <- fitted_models()
    h   <- h_periods()
    req(mdl, h > 0)
  ...
  output$overview_plot <- renderPlot({
    ...
    df |>
      autoplot(Sales) +
      # Gray shading for forecast horizon [cut, end]
      annotate(
        "rect",
        xmin = as.Date(cut),
        xmax = as.Date(end),
  ...
  output$decomp_plot <- renderPlot({
    df <- filtered_data()
    req(nrow(df) > 36)  # min data for STL
    ...
    df |>
      ...
      facet_wrap(~ Varietal, scales = scales_option) +
      labs(
        title = "Seasonal and trend decomposition (STL)",
        x     = "Month"
      ) +
      theme_minimal(base_size = 12) +
      theme(
  ...
  # Forecast visualization (subset by selected models)
